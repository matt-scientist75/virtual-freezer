...
def update_container(container_id: int, payload: schemas.ContainerUpdate, db: Session = Depends(get_db)):
    c = db.query(models.Container).filter(models.Container.id==container_id).first()
    if not c:
        raise HTTPException(status_code=404, detail="Container not found")
...
def update_cell(container_id: int, cell_id: int, payload: schemas.CellUpdate, db: Session = Depends(get_db)):
    ...
    if "vial_name" in payload.__dict__:
        cell.vial_name = payload.vial_name
    if "owner" in payload.__dict__:
        cell.owner = payload.owner
    if "project" in payload.__dict__:
        cell.project = payload.project
    db.commit()
    db.refresh(cell)
    return cell
